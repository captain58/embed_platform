...
{
    return aos_mutex_lock(&stm32_uart[uart_port].uart_mutex, AOS_WAIT_FOREVER);
}
//	int32_t hal_uart_shut(PORT_UART_TYPE uart_port)
//	{
//	    return aos_mutex_unlock(&stm32_uart[uart_port].uart_mutex);
//	}
...
int32_t hal_uart_init(uart_dev_t *uart)
{
    ...
    aos_mutex_new(&stm32_uart[uart->port].uart_mutex);
    ...
    if (ret) {
        ...
        aos_mutex_free(&stm32_uart[uart->port].uart_mutex);
...
int32_t hal_uart_reset(uart_dev_t *uart, uart_config_t * ss)
{
    ...
    {
        pstuarthandle->Init.BaudRate               = ss->baud_rate;
        ret = uart_dataWidth_transform(ss->data_width, &pstuarthandle->Init.WordLength);
        ret |= uart_parity_transform(ss->parity, &pstuarthandle->Init.Parity);
        ret |= uart_stop_bits_transform(ss->stop_bits, &pstuarthandle->Init.StopBits);
        ret |= uart_flow_control_transform(ss->flow_control, &pstuarthandle->Init.HwFlowCtl);
    ...
    memset(stm32_uart[uart->port].UartRxBuf, 0, uartIns->attr.max_buf_bytes);
    stm32_uart[uart->port].uart_rx_in = 0;
 ...
 * @修改人: 
 ************************************************************************/
int32_t hal_uart_to_be_read(uart_dev_t *uart)
{
    int32_t ret = -1;
    UART_MAPPING* uartIns = NULL;
    ...
    if(stm32_uart[uart->port].uart_rx_out != stm32_uart[uart->port].uart_rx_in)
    {
        //CPSR_ALLOC();
        //RHINO_CPU_INTRPT_DISABLE();
    
        ret = (stm32_uart[uart->port].uart_rx_in + 
            uartIns->attr.max_buf_bytes - stm32_uart[uart->port].uart_rx_out) % uartIns->attr.max_buf_bytes;
...
int32_t hal_uart_close(uart_dev_t *uart)
{
    ...
    aos_mutex_unlock(&stm32_uart[uart->port].uart_mutex);
    
    return HAL_UART_DeInit(&stm32_uart[uart->port].hal_uart_handle);
...
SerialID * __Uart_Find(unsigned char ucId)
{
	...
	nCnt = ARR_CNT(gss_UartSID);
	for(i = 0; i < nCnt; i++)
	{
		if(gss_UartSID[i]->handle->port == ucId)
		{
			return (SerialID *)(gss_UartSID[i]);
		}
	}
