    ...
    if(!(str->atr & PST_ATR_PARIT))         //有校验位
    {
        ...
        switch(str->atr & PST_ATR_CSCAL)
        {
            ...
            case PST_ATR_CSCAL_BS:          //位加和
                ...
                break;
            case PST_ATR_CSCAL_BS_ASCII:          //位加和
            {
                uint8_t tmp[128] = {0};
                HexStringToNum(buffer + str->cssp, tmp, sumlen);
                cs = DoBitSum(
                    tmp, 
                    sumlen/2
                    );
                uint8_t tmp_cs = 0;
                HexStringToNum(buffer + sumos, &tmp_cs, 2);
                if((uint8)cs != tmp_cs)
                {
                    return PST_ERR_NE;      //返回不是有效帧      
                }
...
//地址域    
    ...
    {
        if((frmstr->adlp & PST_ADLP_ADD_TO_ASCII) == PST_ADLP_ADD_TO_ASCII)
        {
            uint8_t bcdAddr = 0;
            bcdAddr = ByteHexToBcd(*addr);
            NumToHexString(buf + (frmstr->adlp & PST_ADLP_ADDLEN_MASK),&bcdAddr,1);
        }
        else
        {
            memcpy(buf + (frmstr->adlp & PST_ADLP_ADDLEN_MASK), addr, frmstr->alen);
        }
        if((frmstr->adlp & PST_ADLP_ADDLEN) == PST_ADLP_ADDLEN)
        {
            m+=frmstr->alen;
                        ...
                        NetpFrameDataStr * pDataStr, uint8_t* addr, uint8_t addrlen)
{
    ...
    uint8_t * data = NULL;
    uint16_t cmd = 0;
    uint8_t realAddr[4] = {0};
    if(addr != __NULL)                  //需要判断地址
    {
        //判断接收到的地址是否和要求的一致
        if((pframeStr->adlp & PST_ADLP_ADD_TO_ASCII) == PST_ADLP_ADD_TO_ASCII)
        {
            HexStringToNum(receive + (pframeStr->adlp & PST_ADLP_ADDLEN_MASK),realAddr,addrlen*2);
            uint8_t hexaddr = 0;
            hexaddr = ByteBcdToHex(realAddr[0]);
            if(CmpBuffer(addr, &hexaddr, addrlen) != 0)
            {
        ...
        {
            if(CmpBuffer(addr, receive + (pframeStr->adlp & PST_ADLP_ADDLEN_MASK), addrlen) != 0)
            {
                return PST_ERR_FT;          //返回发生错误
            }
        }
...
//	    inpLbuf->len = DLMS_PackRead(inpLbuf, inpSbuf, stpActCmd);
    ...
    if(err != 0)
    {
        return NETP_ERR_FT;
                                            ...
                                            //数据传输通讯
    if(Netp_Send(stpActCmd->port + NETP_PORT_NO, inpLbuf->buf, &inpLbuf->len, 5, &ss, &fw->frameStr) != NETP_ERR_OK)
    {
        return NETP_ERR_FT;
    }
                                            //提取数据内容
...
uint16_t Netp_Send_With_Retry2(ST_ACT_INPBUF_L *inpLbuf, uint8_t * data, ST_RDMT_CMD *stpActCmd)
{
    ...
    for(uint8_t uc_i = 0; uc_i < 2; uc_i++)
    {
        err = Netp_CommonRead(inpLbuf, data, stpActCmd);
        if((err == NETP_ERR_OK) || (err == NETP_ERR_NT))
        {
            break;
        }
...
//	    }
    ...
    {                           //ADDRESS(DIVIDED BY TST)
        ...
        curedata.seg.dev = gss_CDATATst[uc_i].info.addr & 0x0f;  
        memcpy(curedata.seg.factory_id, fw->device_name, 4);
        UpdateSysTimeStruct();  //更新系统时标
        curedata.seg.rtime = FMT_UnifyTime(gs_SysTime.YMDhms, false, true);   //保存抄表时间
        aos_msleep(100);
        uint8_t dret = gdw_SaveCureData(time, uc_i, (uint8*)&curedata);  //保存到
        LOG_DEBUG("gdw_SaveCureData[%d] time[%02d:%02d:%02d %02d:%02d] ret = %d\n", 
            uc_i,  time[0], time[1], time[2], time[3], time[4], dret);
    }
    return netflag;
}
