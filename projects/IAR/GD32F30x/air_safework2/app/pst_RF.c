...
{         
    ...
    else if(frm->len > 4)
    {
        ...
        if(frm->recv[4]<9)
        {
            ...
            if(SYS_ERR_OK == ES_LOG_OC(en, level))
            {
                frm->send[4]=0;
            }
            else
 ...
 * @修改人: 
 ************************************************************************/
uint8 RfFrmProc(CHN_ENUM channel, uint8 protocol, uint8* rbuffer, uint16 length, uint8* sbuffer, uint8 sbuflen)
{
...
//    uint16 bak;
    ...
    ES_VAR_CHECK(channel >= PST_CHN_NO);    //参数检查
                                            //通道规约帧判断
