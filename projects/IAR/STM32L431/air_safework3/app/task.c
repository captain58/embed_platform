...
bool Task_LPProc(void)
{
    ...
    if(gss_TaskATimer.atr & TKT_LPATR_FLAG)
    {
...
//                  gs_SysVar.mLPstt &= ~HLV_LPTASK_CP;
            ...
            memcpy(&dataTime, pcurtime, sizeof(TIME));
            dataTime.min -= dataTime.min % gs_PstPara.cp_interval;
            uint32_t ndTimes = CalcMins(&dataTime.min, 0);
            uint32_t ncurTimes = CalcMins(&pcurtime->min, 0);
            ndTimes -= 60;
            if(ndTimes % gs_PstPara.cp_interval == 0)
            {
//	                CalcTimeFromMins(dataTime->min, ndTimes + gs_PstPara.cp_interval);
                //ndTimes += gs_PstPara.cp_interval;

            }
            else
            {
                ndTimes = ndTimes + (gs_PstPara.cp_interval - ndTimes % gs_PstPara.cp_interval);
            }
            uint8_t YMDhms[6];
            
            int i = 0; 

            //for(int i = 0; i < (60 / gs_PstPara.cp_interval); i ++)
            do
            {
                CalcTimeFromMins(&dataTime.min, ndTimes);
                MoveBuffer(&dataTime, YMDhms, 6);
                InvBuffer(YMDhms, 6);            
                
                ByteArrayHexToBcd(YMDhms, time, 6);
                ...
                if(ndTimes >= ncurTimes)
                {
                    break;
                }
                i ++;
            }while( i < (60 / gs_PstPara.cp_interval));
