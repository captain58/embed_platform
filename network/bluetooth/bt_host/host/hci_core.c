...
static int le_init(void)
{
    ...
    if (BT_FEAT_BREDR(bt_dev.features)) {
        buf =
          bt_hci_cmd_create(BT_HCI_OP_LE_WRITE_LE_HOST_SUPP, sizeof(*cp_le));
        if (!buf) {
            return -ENOBUFS;
        }
    ...
    if (IS_ENABLED(CONFIG_BT_CONN) && BT_FEAT_LE_DLE(bt_dev.le.features)) {
