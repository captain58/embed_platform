...
typedef struct
{
    ...
    uint8                         doing_smartconfig;
    uint8                         ip_available;
    netmgr_wifi_scan_result_cb_t cb;
    uint8                         wifi_scan_complete_cb_finished;
} netmgr_cxt_t;
...
static uint8 g_station_is_up = false;
...
static void start_mesh(uint8 is_leader)
{
#ifdef CONFIG_AOS_MESH
    node_mode_t mode;
...
uint8 netmgr_get_ip_state()
{
    return g_netmgr_cxt.ip_available;
}

uint8 netmgr_get_scan_cb_finished()
{
...
int netmgr_wifi_start(uint8 autoconfig)
{
    stop_mesh();
