...
typedef enum {
    ...
    OUTPUT_PUSH_PULL_PULL_UP,          /* Output actively driven high and actively driven low -
                                  must not be connected to other active outputs - e.g. LED output */
    OUTPUT_PUSH_PULL_PULL_DOWN,          /* Output actively driven high and actively driven low -
                                  must not be connected to other active outputs - e.g. LED output */
    ...
    OUTPUT_OPEN_DRAIN_PULL_UP, /* Output actively driven low and is pulled high
                                  ...
                                  can be connected to other open-drain/open-collector outputs. */
    OUTPUT_OPEN_DRAIN_PULL_DOWN, /* Output actively driven low and is pulled high
                                  with an internal resistor when set high -
                                  can be connected to other open-drain/open-collector outputs. */
 ...
 * @param[in]  gpio   the gpio pin which should be read
 *
 * @return -1:failed 0 : low 1:high
 */
int32_t hal_gpio_input_get_stt(gpio_dev_t *gpio);
