...
#define LOG_INFOR    6 /* informational message */
#define LOG_DBG     7 /* debug-level message */
#define LOG_NONE    8 /* used in stop filter, all log will pop out */

#define  LOG_LEVEL_NONE		LOG_NONE		
#define  LOG_LEVEL_ERROR	LOG_ERR
#define  LOG_LEVEL_WARN		LOG_WARNING
#define  LOG_LEVEL_INFO		LOG_INFOR
#define  LOG_LEVEL_DEBUG	LOG_DBG
...
typedef enum {
    ...
    AOS_LL_INFO  = LOG_INFOR,    /* info + warn + error log will output */
    AOS_LL_DEBUG = LOG_DBG,   /* debug + info + warn + error + fatal log will output */
} aos_log_level_t;
...
unsigned char rt_log_hex(unsigned char ucLevel, const char *strTitle, const unsigned char *strData, int nLen);


#define LOG_DEBUG(format, args...) 	        rt_log(LOG_DBG, format, ## args)
#define LOG_INFO(format, args...) 	        rt_log(LOG_INFOR, format, ## args)
#define LOG_WARN(format, args...) 	        rt_log(LOG_WARNING, format, ## args)
#define LOG_ERROR(format, args...) 	        rt_log(LOG_ERR, format, ## args)

#define LOG_DUMPHEX(ucLevel, strTitle, strData, nLen)	rt_log_hex(ucLevel, strTitle, strData, nLen)
 ...
 * @param[in]  fmt  same as printf() usage.
 */
#define LOGI(mod, ...) rt_log(LOG_INFOR, mod, __FILE__, __LINE__,  __VA_ARGS__)
 ...
 */
#ifdef DEBUG
#define LOGD(mod, ...) rt_log(LOG_DBG, mod, __FILE__, __LINE__,  __VA_ARGS__)
#else
#define LOGD(mod, ...)
#endif
#else
 ...
 */
#if SYNC_LOG_MOD
#define LOGI(mod, ...) rt_log(LOG_INFOR, mod, __VA_ARGS__)
#else
#define LOGI(mod, ...) rt_log(LOG_INFOR, __VA_ARGS__)
#endif
...
#ifdef DEBUG
#if SYNC_LOG_MOD
#define LOGD(mod, ...) rt_log(LOG_DBG, mod, __VA_ARGS__)
#else
#define LOGD(mod, ...) rt_log(LOG_DBG, __VA_ARGS__)
#endif
