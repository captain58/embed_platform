...
#endif
{
    ...
    if (log_init && s < stop_filter_level) {
        ...
        if (log_get_mutex()) {
            ...
            printf("[%08d]<%c> ", (int)(aos_now_ms()/1000), serverity_name[s]);
 ...
 * @修改说明: 
 ******************************************************************************/
unsigned char rt_log_hex(unsigned char s, const char *strTitle, const unsigned char *strData, int nLen)
{
