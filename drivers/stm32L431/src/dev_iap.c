...
** Description: 该文件需与boot文件操作对应,应用程序只负责往boot指定的flash地址写入升级文件.
...
//外部flash相关宏定义
#define FLS_PROGRAM_ADDR         4096       //程序在外部flash中的位置(前32字节用来存放upatemark)
...
void SYS_APP_IAP_Mark(bool setorclr, uint32 length, uint32 address)
{
    ...
    {
                                        //拷贝写入标记
        memcpy_s(uc_data, (void*)gul_UpdataMarkA_APP, sizeof(gul_UpdataMarkA_APP));
        uc_data[16] = 0;               //数据ID
                                        //长度
        ...
        LongToByte(uc_data + 22, address);
                                        //计算CRC校验
        IntToByte(uc_data + 26, DoCrcDlms(uc_data, 26, 0x5555));
    }
    else                                //仅仅拷贝数据就可以了
    {
        memcpy_s(uc_data, (void*)gul_UpdataMarkB_APP, sizeof(gul_UpdataMarkB_APP));
    }
    ...
    while(length > 0)                   //不断的读取数据然后进行校验
    {
        ...
        else                            //否则读取剩余数据的长度
        {
            SYS_FILE_Read(uc_data, curAdress, length);
                                        //最后的4个字节是CRC校验
            crc = DoCrcDlms(uc_data, length - 4, crc);
                                        //获取CRC校验的对比值
