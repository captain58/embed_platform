...
#include "modemlib.h"
#include "msgid.h"
#define TAG "ec20_gprs_module"

#define EC20_AT_CMD_SUCCESS_RSP "OK"
#define EC20_AT_CMD_FAIL_RSP "ERROR"

#define AT_CMD_TEST "AT"
#define AT_CMD_TEST_RESULT "OK"
#define AT_CMD_EC20_TEST "AT+CGMM"
#define AT_CMD_EC20_TEST_RESULT "EC20F\r\n"
...
#define AT_CMD_EC20_PWR_DOWN "AT+QPOWD"
#define AT_CMD_EC20_CGATT_1 "1"
...
#define EC20_AT_CMD_FTP_SUCCESS_RSP "CONNECT"
#define EC20_AT_CMD_CME_ERROR       "+CME ERROR"
...
#define EC20_DEFAULT_CMD_LEN    64
#define EC20_DEFAULT_RSP_LEN    64
//0~11
#define EC20_MAX_LINK_NUM       12

#define EC20_DOMAIN_MAX_LEN     256
#define EC20_DOMAIN_RSP_MAX_LEN 512
#define EC20_DOMAIN_CMD_LEN (sizeof(AT_CMD_DOMAIN_TO_IP) + EC20_DOMAIN_MAX_LEN + 1)

#define EC20_CONN_CMD_LEN   (EC20_DOMAIN_MAX_LEN + EC20_DEFAULT_CMD_LEN)

#define EC20_RETRY_MAX          3
...
static char  g_pcdomain_rsp[EC20_DOMAIN_RSP_MAX_LEN];
static char  g_pccmd[EC20_CONN_CMD_LEN];
...
static void ec20_gprs_qiopen_rsp_callback(void *arg, char *rspinfo, int rsplen)
{
...
static void ec20_gprs_qftpopen_rsp_callback(void *arg, char *rspinfo, int rsplen)
{
    int i = 0;
    do {
        at_read(&g_pcdomain_rsp[i], 1);
...
static void ec20_gprs_module_socket_data_handle(void *arg, char *rspinfo, int rsplen)
{
    ...
    at_read(&uclinkid, 1);
    linkid = uclinkid - '0';
    if (linkid < 0 || linkid >=  EC20_MAX_LINK_NUM) {
...
//	static void ec20_gprs_module_ftp_data_handle(void *arg, char *rspinfo, int rsplen)
//	{
//	    unsigned char uclinkid = 0;
//	    unsigned char unusesymbol = 0;
//	    unsigned char datalen[6] = {0};
...
static int ec20_send_with_retry(const char *cmd, int cmdlen, bool delimiter, const char *data,
...
static int ec20_send_with_retry2(const char *cmd, int cmdlen, bool delimiter, const char *data,
...
int ec20_uart_selfadaption(const char *command, const char *rsp, uint32_t rsplen)
{
    char buffer[EC20_DEFAULT_RSP_LEN] = {0};
    ...
    if (ec20_send_with_retry(command, strlen(command), true, NULL, 0,
        buffer, EC20_DEFAULT_RSP_LEN, rsp, 1) < 0) {
        return -1;
    }
...
{
    int ret = 0;
    char cmd[EC20_DEFAULT_CMD_LEN] = {0};
    char rsp[EC20_DEFAULT_RSP_LEN] = {0};
    
    memset(g_pcdomain_rsp , 0, EC20_DOMAIN_RSP_MAX_LEN);
    memset(g_link, 0, sizeof(g_link));
    
    at_reset_uart();

    for (int linknum = 0; linknum < EC20_MAX_LINK_NUM; linknum++) {
        g_link[linknum].fd = -1;
    }

    /* uart baudrate self adaption*/
//	    ret = ec20_uart_selfadaption(AT_CMD_TEST, AT_CMD_TEST_RESULT, strlen(AT_CMD_TEST_RESULT));
//	    if (ret) {
//	        LOG_INFO( "ec20_uart_selfadaption fail \r\n");
//	        return ret;
//	    }
    /*turn off echo*/
    at_send_wait_reply(AT_CMD_ECHO_OFF, strlen(AT_CMD_ECHO_OFF), true,
                       NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
    ...
    /*set baudrate 115200*/
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%d", AT_CMD_BAUDRATE_SET, AT_UART_BAUDRATE);
    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0,
                       rsp, EC20_DEFAULT_RSP_LEN, NULL);
    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
...
//	    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
//	    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
//	    /*turn off flow control*/
//	    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%d,%d", AT_CMD_FLOW_CONTROL, 0, 0);
//	    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0,
//	                       rsp, EC20_DEFAULT_RSP_LEN, NULL);
//	    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
//	        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
...
{
    char cmd[EC20_DEFAULT_RSP_LEN] = {0};
    char rsp[EC20_DEFAULT_RSP_LEN] = {0};
    int i = 0;
    if(bit & 0x01)
    {
        /*sim card status check*/
        if (ec20_send_with_retry(AT_CMD_SIM_PIN_CHECK, strlen(AT_CMD_SIM_PIN_CHECK), true,
            NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
...
//	    LOG_INFO( "network registration is %s \r\n", rsp);
    if(bit & 0x04)
    {
        memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
        memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
        snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s", AT_CMD_STATUS_CONTROL_CHECK,"\"gprsattach\"");
        if (ec20_send_with_retry(cmd, strlen(cmd), true,
            NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
            LOG_INFO( "%s failed\n", cmd);
            return -1;
        }
...
//	        memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
//	        memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
//	        snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s", AT_CMD_STATUS_CONTROL_CHECK,"\"nwscanmode\"");
//	        if (ec20_send_with_retry(cmd, strlen(cmd), true,
//	            NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
//	            LOG_INFO( "%s failed\n", cmd);
//	            return -1;
...
//	        memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
//	        memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
//	        snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s", AT_CMD_STATUS_CONTROL_CHECK,"\"nwscanseq\"");
//	        if (ec20_send_with_retry(cmd, strlen(cmd), true,
//	            NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
//	            LOG_INFO( "%s failed\n", cmd);
//	            return -1;
...
//	    aos_msleep(3000);
    ...
    if(bit & 0x08)
    {
        for(i = 0; i < CON_CSQ_CREG_RETRY_TIMES; i++)
        {
            ...
            aos_msleep(5000);
            if (ec20_send_with_retry2(AT_CMD_NETWORK_REG_CHECK, strlen(AT_CMD_NETWORK_REG_CHECK), true,
                NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, CREG_SUCC_STR, 2, 0) < 0) {
        ...
        for(i = 0; i < CON_CSQ_CREG_RETRY_TIMES; i++)
        {
            ...
            memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
            /*GPRS attach check*/
            if (ec20_send_with_retry(AT_CMD_GPRS_ATTACH_CHECK, strlen(AT_CMD_GPRS_ATTACH_CHECK), true,
                NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, AT_CMD_EC20_CGATT_1, 0) < 0) {
...
int ec20_gprs_ip_init(void)
{
    ...
    if (ec20_send_with_retry(AT_CMD_GPRS_PDP_DEACTIVE, strlen(AT_CMD_GPRS_PDP_DEACTIVE), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
        LOG_INFO( "Deactivate GPRS PDP Context failed\n");
        
        return -1;
    }
    if (ec20_send_with_retry(AT_CMD_GPRS_PDP_ACTIVE, strlen(AT_CMD_GPRS_PDP_ACTIVE), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
        LOG_INFO( "Deactivate GPRS PDP Context failed\n");
    ...
    /*get operator*/
    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
    if (ec20_send_with_retry(AT_CMD_MISI, strlen(AT_CMD_MISI), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
    ...
    if(strstr(rsp,"46000") || strstr(rsp,"46002") || strstr(rsp,"46007") || strstr(rsp,"46004"))
    {
        ...
        snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=1,1,\"%s\",\"\",\"\"", AT_CMD_APN, AT_CMD_APN_CHINAMOBILE);
    }
    else if(strstr(rsp,"46001") || strstr(rsp,"46006"))
    {
        
        snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=1,1,\"%s\",\"\",\"\"", AT_CMD_APN, AT_CMD_APN_CHINAUNINET);
    }
    else
    {
    
        snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=1,1,\"%s\",\"\",\"\"", AT_CMD_APN, AT_CMD_APN_CHINATELECOM);
...
//	    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
//	    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=1", AT_CMD_START_CLIENT_ACT);
//	    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
//	    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
...
//	    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
//	    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%d", AT_CMD_MULTI_IP_CONNECTION, 1);
//	    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
//	    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
//	        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
//	        return -1;
...
//	    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
//	    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
//	    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%d", AT_CMD_SEND_DATA_PROMPT_SET, 0);
//	    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
//	    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
//	        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
//	        return -1;
...
//	    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
//	    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
//	    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%d", AT_CMD_RECV_DATA_FORMAT_SET, 1);
//	    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
//	    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
//	        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
//	        return -1;
//	    }
    ...
    at_register_callback(AT_CMD_CLIENT_CONNECT_RET, NULL, EC20_DEFAULT_RSP_LEN,
                         ec20_gprs_qiopen_rsp_callback, NULL);
    at_register_callback(AT_CMD_DATA_RECV, NULL, 0, ec20_gprs_module_socket_data_handle, NULL);


    return 0;
}

static void ec20_get_ip_delayed_action(void *arg)
{
...
int ec20_gprs_got_ip(uint8_t * ip)
{
    ...
    /*start gprs stask*/
    if (ec20_send_with_retry(AT_CMD_START_TASK, strlen(AT_CMD_START_TASK), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        return -1;
    }
...
//	                       NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, &atcmd_config);
//	    if (strstr(rsp, EC20_AT_CMD_FAIL_RSP) != NULL) {
//	        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
//	    }
//		
//	    /*delay 5 seconds to post got ip event*/
//	    aos_post_delayed_action(5000, ec20_get_ip_delayed_action, NULL);
    //return 0;
}

static int ec20_gprs_get_ip_only()
{
    char rsp[EC20_DEFAULT_RSP_LEN] = {0};
    atcmd_config_t atcmd_config = {NULL, AT_RECV_PREFIX, NULL};
    at_send_wait_reply(AT_CMD_GOT_LOCAL_IP, strlen(AT_CMD_GOT_LOCAL_IP), true,
...
int32_t ec20_get_gprs_network_info(char * bts, int btslen, char * neighterbts, int nbtslen)
{
    ...
    if (!inited)
    {
        LOG_INFO( "%s ec20 gprs module haven't init yet \r\n", __func__);
    }
...
void ec20_get_gps(float * latitude, float * longitude, float * altitude)
{
    int ret = 0;
    ...
    *altitude = 0;
    ...
    if ((0 != ret) || (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL))
    {
...
//	
//	    /*turn off */
//	    if (ec20_send_with_retry(AT_CMD_EC20_PWR_DOWN, strlen(AT_CMD_EC20_PWR_DOWN), true,
//	        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, 0) < 0) {
//	        LOG_INFO( "ec20 Power off failed\n");
//	        
//	          hal_gpio_output_high(&brd_gpio_table[GPIO_MODEM_ONOFF]);//_ATMODEM_ONOFF_1();
//	          aos_msleep(1000);
//	          hal_gpio_output_low(&brd_gpio_table[GPIO_MODEM_ONOFF]);//_ATMODEM_ONOFF_0();
...
int ec20_modemcheck(void)
{
    ...
    /* uart baudrate self adaption*/
    //at_reset_uart();
    ret = ec20_uart_selfadaption(AT_CMD_EC20_TEST, AT_CMD_EC20_TEST_RESULT, strlen(AT_CMD_TEST_RESULT));
    if (ret) {
        LOGE(TAG, "ec20_uart_selfadaption fail \r\n");
        return ret;
    }
...
int ec20_domaintoip(char *domain, char ip[16])
{
    ...
    if (strlen(domain) > EC20_DOMAIN_MAX_LEN) {
    ...
    pccmd = g_pccmd;
    if (NULL == pccmd) {
        LOG_INFO( "fail to malloc memory %d at %s \r\n", EC20_DOMAIN_CMD_LEN, __func__);
        return -1;
    }

    memset(pccmd, 0, EC20_DOMAIN_CMD_LEN);
    snprintf(pccmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s", AT_CMD_DOMAIN_TO_IP, domain);

    aos_mutex_lock(&g_domain_mutex, AOS_WAIT_FOREVER);
restart:
    count++;
    if (count > EC20_RETRY_MAX) {
        LOG_INFO("domain to ip retry failed!\r\n");
        aos_mutex_unlock(&g_domain_mutex);//HAL_MutexUnlock(g_domain_mutex);
        return -1;
    }

    at_send_wait_reply(pccmd, strlen(pccmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        goto err;
    }
...
//	        g_link[linkid].fd = conn->fd;
    ...
    pccmd = g_pccmd;
    if (NULL == pccmd) {
        LOG_INFO( "fail to malloc %d at %s \r\n", EC20_CONN_CMD_LEN, __func__);
        goto err;
    }
    memset(pccmd, 0, EC20_CONN_CMD_LEN);

//	    snprintf(pccmd, EC20_CONN_CMD_LEN - 1, "%s=%d", AT_CMD_START_CLIENT_DEACT, linkid);
//	    at_send_wait_reply(pccmd, strlen(pccmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
//	    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
//	        LOG_INFO( "pccmd %s fail, rsp %s \r\n", pccmd, rsp);
//	        goto err;
//	    }
    ...
    switch (conn->type) {
        case TCP_SERVER:
//	            snprintf(pccmd, EC20_CONN_CMD_LEN - 1, "%s=%d,%d", AT_CMD_START_TCP_SERVER, 1, conn->l_port);
            snprintf(pccmd, EC20_CONN_CMD_LEN - 1, "%s=1,%d,\"TCP LISTENER\",\"127.0.0.1\",0,%d,1", AT_CMD_START_CLIENT_CONN, linkid, 
                     conn->r_port);
        ...
        case TCP_CLIENT:
          {
            snprintf(pccmd, EC20_CONN_CMD_LEN - 1, "%s=1,%d,\"TCP\",\"%s\",%d,0,1", AT_CMD_START_CLIENT_CONN, linkid, conn->addr,
                     conn->r_port);
//	            uint8_t suc[EC20_DEFAULT_RSP_LEN];
//	            
//	            memset(suc, 0, EC20_DEFAULT_RSP_LEN);
//	            snprintf(suc, EC20_CONN_CMD_LEN - 1, "%d,0",  linkid);
            //at_set_timeout(600000);
            atcmd_config_t atcmd_config_client = { NULL, EC20_AT_CMD_SUCCESS_RSP, NULL};
            //atcmd_config_client.reply_success_postfix = &suc[0];
            at_send_wait_reply(pccmd, strlen(pccmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, &atcmd_config_client);
...
//	            }
            ...
            if(ret != 0 ||
                DecodeStrOneChar(g_pcdomain_rsp,&retid)!=0 || retid >= EC20_MAX_LINK_NUM ||
...
//	            at_set_timeout(10000);
        ...
        case UDP_UNICAST:
          {
//	            snprintf(pccmd, EC20_CONN_CMD_LEN - 1, "%s=%d,\"UDP\",\"%s\",%d", AT_CMD_START_CLIENT_CONN, linkid, conn->addr,
//	                     conn->r_port);
            snprintf(pccmd, EC20_CONN_CMD_LEN - 1, "%s=1,%d,\"UDP\",\"%s\",%d,0,1", AT_CMD_START_CLIENT_CONN, linkid, conn->addr,
                     conn->r_port);
...
//	            }
        ...
        case UDP_BROADCAST:
        default:
            LOG_INFO( "ec20 gprs module connect type %d not support \r\n", conn->type);
            goto err;
    }
...
int ec20_close(int fd, int32_t remote_port)
{
    ...
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%d", AT_CMD_STOP_CONN, linkid);
    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
                 ...
                 int32_t timeout)
{
    ...
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%d,%d", AT_CMD_SEND_DATA, linkid, len);



    /*TODO data send fail rsp is SEND FAIL*/
//	    if (ec20_send_with_retry((const char *)cmd, strlen(cmd), true, NULL, 0,
//	        rsp, sizeof(rsp), ">", 0) < 0) {
...
//	    }
    ...
    at_send_wait_reply((const char *)cmd, strlen(cmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, &atcmd_config_client);
    if (strstr(rsp, ">") == NULL) {
    ...
    at_send_wait_reply(data, len, true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, NULL);
...
int ec20_ftp_status_check(uint8_t * usr, uint8_t * pwd)
{
    ...
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s", AT_CMD_FTP_CFG,AT_CMD_FTP_CFG_CNT);
    if (ec20_send_with_retry(cmd, strlen(cmd), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
    ...
    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s,\"%s\",\"%s\"", AT_CMD_FTP_CFG,AT_CMD_FTP_CFG_ACOUNT,usr,pwd);
    if (ec20_send_with_retry(cmd, strlen(cmd), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
    ...
    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s", AT_CMD_FTP_CFG,AT_CMD_FTP_CFG_FT);
    if (ec20_send_with_retry(cmd, strlen(cmd), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
    ...
    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s", AT_CMD_FTP_CFG,AT_CMD_FTP_CFG_MD);
    if (ec20_send_with_retry(cmd, strlen(cmd), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
    ...
    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=%s", AT_CMD_FTP_CFG,AT_CMD_FTP_CFG_TO);
    if (ec20_send_with_retry(cmd, strlen(cmd), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
    ...
    at_register_callback(AT_CMD_FTP_CONNECT_RET, NULL, EC20_DEFAULT_RSP_LEN,
                         ec20_gprs_qftpopen_rsp_callback, NULL);
    at_register_callback(AT_CMD_FTP_FOLD_RET, NULL, EC20_DEFAULT_RSP_LEN,
                         ec20_gprs_qftpopen_rsp_callback, NULL);

    at_register_callback(AT_CMD_FTP_GET_RET, NULL, EC20_DEFAULT_RSP_LEN,
                         ec20_gprs_qftpopen_rsp_callback, NULL);
...
int ec20_ftp_start(uint8_t * domain, uint16_t port, uint8_t * fold)
{
    ...
    snprintf(pccmd, EC20_CONN_CMD_LEN - 1, "%s=\"%s\",%d", AT_CMD_START_FTP, domain, port);
    atcmd_config_t atcmd_config_client = { NULL, EC20_AT_CMD_SUCCESS_RSP, EC20_AT_CMD_CME_ERROR};
    at_send_wait_reply(pccmd, strlen(pccmd), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, &atcmd_config_client);
    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
...
//	                    ec20_gprs_module_ftp_data_handle, NULL);
    memset(pccmd, 0, EC20_CONN_CMD_LEN);

    memset(g_pcdomain_rsp,0,64);
    //\"/\"

    snprintf(pccmd, EC20_CONN_CMD_LEN - 1, "%s=\"/%s/\"", AT_CMD_FTP_FOLD_CFG, fold);

    if (ec20_send_with_retry(pccmd, strlen(pccmd), true,
        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
...
#define EC20_FTP_BUF_MAX_LEN 1100
uint8_t ftp_buf[EC20_FTP_BUF_MAX_LEN];
...
int ec20_ftp_get(uint8_t * file, uint8_t * outbuf, uint32_t offset, uint16_t * len)
{
    ...
    memset(rsp, 0, EC20_DEFAULT_RSP_LEN);
    memset(cmd, 0, EC20_DEFAULT_CMD_LEN);
    snprintf(cmd, EC20_DEFAULT_CMD_LEN - 1, "%s=\"%s\",\"COM:\",%d,%d", AT_CMD_FTP_GET,file,offset,buf_len);
//	    if (ec20_send_with_retry(cmd, strlen(cmd), true,
//	        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_FTP_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
//	        LOG_INFO( "ftp rsp time out [%s]\n", rsp);
//	        return -1;
//	    }
    //at._timeout = 20000;
    memset(g_pcdomain_rsp,0,64);
    memset(ftp_buf,0,EC20_FTP_BUF_MAX_LEN);
    uint8_t str_ok[EC20_DEFAULT_CMD_LEN];
    memset(str_ok,0,EC20_DEFAULT_CMD_LEN);
    snprintf(str_ok, EC20_DEFAULT_CMD_LEN - 1, "%s\r\n", "OK");
    atcmd_config_t atcmd_config_client = { NULL, str_ok, "+CME ERROR:"};
    at_reset_uart();
    at_send_wait_reply(cmd, strlen(cmd), true,
                       NULL, 0, ftp_buf, EC20_FTP_BUF_MAX_LEN, &atcmd_config_client);
    if (strstr(ftp_buf, EC20_AT_CMD_FTP_SUCCESS_RSP) == NULL) {
        LOG_INFO( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
    ...
    uint8_t * pbuf = strstr(ftp_buf,EC20_AT_CMD_FTP_SUCCESS_RSP);
    if(NULL != pbuf)
    {
        pbuf+=9;
    }
...
//	    if (ec20_send_with_retry(AT_CMD_STOP_FTP, strlen(AT_CMD_STOP_FTP), true,
//	        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
//	        LOG_INFO( "cmd %s rsp is %s \r\n", AT_CMD_STOP_FTP, rsp);
//	        return -1;
//	    }
    atcmd_config_t atcmd_config_client = { NULL, EC20_AT_CMD_SUCCESS_RSP, EC20_AT_CMD_CME_ERROR};
    at_send_wait_reply(AT_CMD_STOP_FTP, strlen(AT_CMD_STOP_FTP), true, NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, &atcmd_config_client);
    if (strstr(rsp, EC20_AT_CMD_SUCCESS_RSP) == NULL) {
        LOG_INFO( "cmd %s fail, rsp %s \r\n", AT_CMD_STOP_FTP, rsp);
        return -1;
    }

//	    if (ec20_send_with_retry(AT_CMD_STOP_FTP_DEACT, strlen(AT_CMD_STOP_FTP_DEACT), true,
//	        NULL, 0, rsp, EC20_DEFAULT_RSP_LEN, EC20_AT_CMD_SUCCESS_RSP, EC20_RETRY_MAX) < 0) {
